...
class FinalTables(object):
    ...
    def original_table(self, table_name, filter="", database_name=None):
        ...
        query = "SELECT * FROM `{0}`.`{1}` {2}".format(
            original_database, table_name, filter
        )
    ...
    def insert_into_final(self, output_table_name, rows, target_database=None):
        ...
        self.sql.execute(
            "TRUNCATE `{0}`.`{1}`".format(final_database, output_table_name)
        )
        columns = rows[0].keys()
        values = []
        for row in rows:
    ...
    def populate_story_tags(self, story_id, output_table_name, story_tags):
        ...
        for col, tags in story_tags.items():
            cols_with_tags.append(
                "{0}='{1}'".format(col, tags.replace("'", "\\'").strip())
            )
    ...
    def story_to_final_without_tags(self, story, story_authors, is_story=True):
        ...
        if authors_count > 2:
            ...
            self.log.warning(
                f"{type} {story['id']} has {authors_count} authors - listing all authors in notes..."
            )
            story_authors_ids = [str(x["author_id"]) for x in story_authors]
            author_names = "Creators: {} and {}".format(
                ", ".join(story_authors_ids[:-1]), story_authors_ids[-1]
            )
        ...
        if is_story:
            ...
            final_story["coauthor_id"] = (
                story_authors[1]["author_id"] if authors_count > 1 else None
            )
        return final_story
