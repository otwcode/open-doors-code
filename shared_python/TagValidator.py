class TagValidator(object):
    ...
    dict_category = {"Gen": 1, "F/M": 2, "M/M": 3, "F/F": 4, "Multi": 5, "Other": 6}
    ...
    def print_tag_warning(self, tag, tag_type, isType):
        if isType:
            ...
            self.log.warning(
                ...
                + '" is not a valid '
                + tag_type.upper()
                + " tag. Attempting self correction..."
            )

    def print_fail_self(self):
        self.log.error(
            "All attempts at self correction have failed."
    ...
    def print_fail(self, tag_name):
        if tag_name:
            self.log.error(
                'Manual Input Failed. "' + tag_name + '" has failed re-check.'
            )
        else:
